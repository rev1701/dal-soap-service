...
namespace LMS1701.EA.SOAPAPI
{
    ...
    public class Service1 : IService1
    {
        ...
        public List<ExamQuestion> GetAllExamQuestion()
        {
            ...
            var dbExamQuestion = db.ExamQuestions.ToList();
            var dbCategories = db.Categories.ToList();
            var dbSubtopic = db.Subtopics.ToList();
            var dbCatSub = db.Categories_Subtopic.ToList();
            var dbQuestExam = db.ExamQuestionLists.ToList();
            var dbQuestion = db.Questions.ToList();
        ...
        public List<Question> GetAllQuestions()
        {
            ...
            var first = from c in db.Questions
                        select c;
        ...
        public List<Answers> GetAnswersQuestion(int Questid)
        {
            ...
            List<EAD.Answer> AnswerDB = db.Answers.ToList();
            List<EAD.QuestionAnswer> dbQuestionAns = db.QuestionAnswers.ToList();
        ...
        public ExamTemplate getExamTemplate(String id)
        {
            ...
            #endregion
            List<EAD.ExamTemplate> ExamTemplate = db.ExamTemplates.Where(s => s.ExamTemplateID == id).ToList();
            ...
            #endregion
            List<EAD.ExamTemplateQuestion> ExamQuestions = db.ExamTemplateQuestions.Where(s => s.ExamTemplateID == exam.ExamTemplateID).ToList();
            ...
            {
                var dbExamQuestion = db.ExamQuestions.ToList();
                var dbExamQuestionList = db.ExamQuestionLists.ToList();
                var dbQuestionAnswers = db.QuestionAnswers.ToList();
                var dbquestion = db.Questions.ToList();
                var dbanswer = db.Answers.ToList();
                for (int i = 0; i < ExamQuestions.Count(); i++)
                {
                    ...
                    for (int j = 0; j < ExamQuestionList.Count(); j++)
                    {
                        ...
                        #endregion
                        List<EAD.QuestionAnswer> AnswersID = db.QuestionAnswers.Where(s => s.QuestionID == quest.PKID).ToList();
        ...
        public List<Subject> GetAllSubject()
        {
            ...
            /* var subjects = from tempSubjects in db.Subject
                     select tempSubjects; */
            #endregion
            List<EAD.Subject> subjects = db.Subjects.ToList();
            ...
            for (int i = 0; i < subjects.ToList().Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        List<EAD.Category> categoriesL = db.Categories.Where(c => c.Categories_ID == categories.ElementAt(b).Categories_ID).ToList();
                        ...
                        else
                        {
                            for (int c = 0; c < SubtopicIDs.ToList().Count; c++)
                            {
                                ...
                                List<EAD.Subtopic> Subtopics = db.Subtopics.Where(s => s.Subtopic_ID == SubtopicIDs.ToList().ElementAt(c)).ToList();
                                newSub = new SubTopic();
        ...
        */
        ...
        public void AddAnswer(int QuestionID, string Answer, bool IC)
        {
            ...
            try
            {
                db.Answers.Add(ans);
                db.SaveChanges();
                var tempPKID = db.Answers.OrderByDescending(item => item.PKID).First();
        ...
        public void DeleteAnswer(string Answerdesc)
        {
            ...
            foreach (var item in db.Answers) //Gets the Answer which will be needed so it can be removed
            {
            ...
            db.Answers.Remove(removedAnswer); // removes the Answer from the Answer table.
            db.SaveChanges();
        }
        ...
        public void DeleteExam(string ExamTID)
        {
            ...
            foreach (var item in db.ExamTemplates) //Gets the Exam which will be needed so it can be removed
            {
            ...
            db.ExamTemplates.Remove(removedExam); // removes the ExamTemplate from the ExamTemplate table.
            db.SaveChanges();
        }
        ...
        public void AddNewExam(string exName, string exTID, string ExamType)
        {
            ...
            foreach (var item in db.ExamTypes)
            {
        ...
        {
            foreach (var item in db.ExamTemplates)
            {
        ...
        public void DeleteSubtopic(string SubtopicName)
        {
            ...
            foreach (var item in db.Subtopics) //Gets the subtopicID which will be needed so it can be removed
            {
            ...
            db.Subtopics.Remove(removedTopic); // removes the subtopic from the subtopic table.
            db.SaveChanges();
        }
        ...
        {
            int categoryID = 0;
            EAD.Category removedCategory = new EAD.Category();
            foreach (var item in db.Categories) //Gets the categoryID which will be needed so it can be removed
            {
                if (item.Categories_Name == CategoryName)
                {
        ...
        public void AddSubject(string SubjectName)
        {
            ...
            db.Subjects.Add(addedSubject); //adds object to database
            db.SaveChanges();
        }
        ...
        public void RemoveCategoryFromSubject(string CategoryName, string SubjectName)
        {
            ...
            foreach (var item in db.Subjects) //Gets the subjectID which will be needed so it can be removed
            {
        ...
        public void DeleteSubject(string SubjectName)
        {
            ...
            db.Subjects.Remove(removedSubject); // removes the subject from the subtopic table.
            db.SaveChanges();
        }
